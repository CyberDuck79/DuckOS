...
mkdir /tmp/src
cd /tmp/src
# TO UPDATE (but it work with this version)
...
--with-mpc=/usr/local/Cellar/libmpc/1.2.1 2>&1 | tee configure.log } \
...
&& { make all-target-libgcc 2>&1 | tee make.log } \
&& { make install-gcc 2>&1  | tee make.log } \
&& { make install-target-libgcc 2>&1  | tee make.log } \
